...
export default function About() {
    ...
    let HandleMode=()=>{
        if(mode.color=='black'){
            ...
            changeModeText('Change to light mode')
        }
        else{
            changeMode({
                color:'black',
        ...
        changeModeText('Change to dark mode')
    ...
    const [modeText,changeModeText]=useState("Change to light mode");
  ...
  return (
    <div className='container my-3' style={mode}>
        ...
        <button onClick={HandleMode} className="btn btn-primary my-3">{modeText}</button>
        </div>
    </div>
  )
}
